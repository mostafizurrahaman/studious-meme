...
export const metadata: Metadata = buildMetadata({
    ...
    path: '/dashboard/super-admin/coupons',
    noindex: true,
});
...
export default async function SuperAdminCouponsPage({ searchParams }: Props) {
    await requireDashboardRoles(['ADMIN', 'SUPER_ADMIN']);
    const query = await searchParams;
    const page = parsePositiveInteger(query.page, 1);
    const limit = parsePositiveInteger(query.limit, 50);
    const searchTerm = query.searchTerm?.trim() ?? '';
    const couponsResult = await getAllCoupons({ page, limit, searchTerm }).catch(() => null);
    ...
    const paginationMeta = {
        ...
        totalPages: couponsResult?.meta?.totalPages ?? (Math.ceil(coupons.length / limit) || 1),
    };
