          ...
          <div className="mt-4 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-2xl text-sm leading-7 text-white/78 sm:text-base">
                Browse the storefront catalog with filters, categories and product cards
          ...
          <aside className="hidden rounded-3xl bg-white p-5 shadow-sm lg:block">
            ...
            <div className="mt-4 space-y-4 text-sm">
              <div>
                ...
                <div className="mt-2 grid gap-2 text-black/65">
                  {topCategories.map(category => (
                    <Link key={category.name} href={category.href} className="hover:text-[#f15a24]">
                      {category.name}
                    </Link>
                  ))}
                </div>
              ...
              <div>
                ...
                <div className="mt-2 text-black/65">Tk. 500 - Tk. 120,000</div>
              </div>
            </div>
          </aside>
          ...
          <div className="space-y-4">
            <div className="rounded-3xl bg-white p-4 shadow-sm">
              <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                ...
                <div className="flex flex-wrap gap-2 text-xs font-semibold">
                  {['All', 'Sale', 'Featured', 'Latest', 'Industrial', 'Home'].map(tag => (
                    <span key={tag} className="rounded-full bg-[#f5f6f8] px-3 py-2 text-black/75">
                      {tag}
                    </span>
                  ))}
                </div>
            ...
            <div className="flex items-center justify-between rounded-3xl bg-white p-4 text-sm shadow-sm">
              ...
              <div className="flex gap-2">
                <span className="rounded-full bg-[#0e2f56] px-4 py-2 font-semibold text-white">1</span>
                <span className="rounded-full bg-[#f5f6f8] px-4 py-2 font-semibold text-black/70">2</span>
                <span className="rounded-full bg-[#f5f6f8] px-4 py-2 font-semibold text-black/70">3</span>
              </div>
            </div>
          </div>
        </section>
