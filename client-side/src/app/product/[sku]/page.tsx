...
export default async function ProductPage({ params }: Props) {
  ...
  const related = allProducts.filter(item => item.sku !== product.sku).slice(0, 4);
      ...
      <div className="mx-auto w-full max-w-310 px-4 py-6 lg:px-0">
        ...
        <section className="mt-4 grid gap-6 lg:grid-cols-[1.08fr_0.92fr]">
            ...
            <div className="grid gap-4 lg:grid-cols-[92px_minmax(0,1fr)]">
              <div className="order-2 grid grid-cols-4 gap-3 lg:order-1 lg:grid-cols-1">
                {gallery.map((image, index) => (
                  <div key={`${image}-${index}`} className="relative aspect-square overflow-hidden rounded-2xl border border-black/8 bg-[#f5f6f8]">
                    <Image src={image} alt={`${product.title} thumbnail ${index + 1}`} fill sizes="92px" className="object-cover" />
                  </div>
                ))}
              </div>
              <div className="order-1 lg:order-2">
                <div className="relative aspect-square overflow-hidden rounded-3xl bg-[#f5f6f8]">
                  <Image src={product.image} alt={product.title} fill sizes="(max-width: 1024px) 100vw, 60vw" className="object-cover" />
                  {product.badge ? (
                    <span className="absolute left-4 top-4 rounded-full bg-[#f15a24] px-3 py-1 text-xs font-bold text-white shadow-sm">
                      {product.badge}
                    </span>
                  ) : null}
                </div>
          ...
          <div className="rounded-3xl bg-white p-6 shadow-sm">
            <p className="text-xs font-semibold uppercase tracking-[0.28em] text-[#f15a24]">
              Product details
            </p>
            ...
            <div className="mt-4 flex flex-wrap items-center gap-2 text-sm text-black/60">
              <span className="rounded-full bg-[#f5f6f8] px-3 py-1 font-semibold text-black/70">Brand: {product.brand}</span>
              <span className="rounded-full bg-[#f5f6f8] px-3 py-1 font-semibold text-black/70">SKU: {product.sku}</span>
              <span className="rounded-full bg-[#f5f6f8] px-3 py-1 font-semibold text-black/70">{product.stock}</span>
              <span className="rounded-full bg-[#f5f6f8] px-3 py-1 font-semibold text-black/70">Free consultation</span>
            </div>

            <div className="mt-5 flex items-center gap-2 text-sm text-black/60">
              <div className="text-[#f15a24]">★★★★★</div>
            ...
            <div className="mt-6 rounded-3xl border border-black/8 bg-[#f5f6f8] p-5">
              <div className="flex items-end gap-3">
                <div className="text-4xl font-black text-[#f15a24]">{product.price}</div>
                {product.oldPrice ? <div className="pb-1 text-lg text-black/40 line-through">{product.oldPrice}</div> : null}
              </div>
              <div className="mt-2 text-sm text-black/55">Price includes standard store margin and catalog discount.</div>
            </div>

            <p className="mt-5 text-[13px] leading-7 text-black/65">
              Built for workshop buyers and procurement teams, this product detail view keeps the same buying rhythm used across the storefront.
            </p>
        ...
        <section className="mt-6 grid gap-4 lg:grid-cols-[1fr_300px]">
          <div className="rounded-3xl bg-white p-6 shadow-sm">
            <div className="flex flex-wrap gap-2 text-sm font-semibold">
              {['Description', 'Specifications', 'Reviews'].map(tab => (
                <span key={tab} className="rounded-full bg-[#f5f6f8] px-4 py-2 text-black/70">{tab}</span>
              ))}
            </div>
            <div className="mt-6 grid gap-6 lg:grid-cols-2">
              <div>
                <h2 className="text-xl font-black text-[#0e2f56]">Description</h2>
                <p className="mt-3 text-sm leading-7 text-black/65">
                  {product.title} is displayed here with store-ready details, product benefits and a clean purchase path for buyers.
                </p>
                <p className="mt-3 text-sm leading-7 text-black/65">
                  The layout is designed to feel familiar to Malamal shoppers, with the main focus on product image, price, stock and action buttons.
                </p>
              </div>
              <div>
                <h2 className="text-xl font-black text-[#0e2f56]">Specifications</h2>
                <div className="mt-4 overflow-hidden rounded-2xl border border-black/8">
                  {[
                    ...
                    <div key={label} className="grid grid-cols-[180px_1fr] border-b border-black/8 last:border-b-0">
                      <div className="bg-[#f5f6f8] px-4 py-3 font-semibold text-[#0e2f56]">{label}</div>
            ...
            <div className="mt-6 rounded-3xl bg-[#f5f6f8] p-5 text-sm leading-7 text-black/65">
              <p>
                This product page mirrors a shop-first layout with the gallery, price block, description, and specification table arranged for quick buyer decisions.
              </p>
            </div>
          </div>
          <aside className="rounded-3xl bg-[#0e2f56] p-6 text-white shadow-sm">
            ...
            <p className="mt-3 text-sm leading-7 text-white/78">
              Send your quantity and delivery location to receive a quotation for project or retail purchase.
            </p>
            <Link href="/quotation-request" className="mt-6 inline-flex h-11 items-center justify-center rounded-full bg-white px-6 text-sm font-bold text-[#0e2f56]">
              Request quotation
            </Link>
        ...
        <section className="mt-10 rounded-3xl bg-white p-5 shadow-sm sm:p-6">
          <h2 className="text-xl font-black text-[#0e2f56]">Related products</h2>
