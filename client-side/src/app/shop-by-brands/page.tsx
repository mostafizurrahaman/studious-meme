        ...
        <section className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {brands.map(brand => (
            <Link
              key={brand.name}
              href={brand.href}
              className="rounded-3xl bg-white p-6 text-center shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"
            >
              <div className="text-xl font-black text-[#0e2f56]">{brand.name}</div>
