    ...
    <>
      ...
      {products.length > 0 ? (
                ...
                trailingAction={
                  <Button
                    ...
                    onClick={() => {
                      ...
                      remove(product.sku);
                      setBackendProducts(current => current.filter(item => item.sku !== product.sku));

                      if (!product.id) {
                        toast.error('Unable to remove this wishlist item right now.');
                        return;
                      }
                      ...
                      void removeWishlistItem(product.id).then(result => {
                        if (!result.success) {
                          ...
                          setBackendProducts(current =>
                            current.some(item => item.sku === product.sku) ? current : [product, ...current],
                          );
        ...
        <Card className="mt-6 p-8 text-center shadow-sm">
          <div className="text-lg font-black text-primary">No products added to wishlist</div>
