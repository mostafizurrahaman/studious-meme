...
export function ComparePageClient({ authenticated, initialProducts }: Props) {
    ...
    const [backendProducts, setBackendProducts] = useState<Product[]>(initialProducts);
        ...
        return (
            <Card className="p-6 shadow-sm sm:p-8">
                ...
                <p className="mt-3 max-w-3xl text-sm leading-7 text-foreground/65 sm:text-base">
                    Compare is available after sign in so your list always stays tied to your account.
                </p>
        ...
        <>
            <Card className="p-6 shadow-sm sm:p-8">
                ...
                <div className="mt-4 flex flex-wrap items-end justify-between gap-4">
                    <div>
                        <h1 className="text-3xl font-black text-secondary sm:text-4xl">Compare</h1>
                        <p className="mt-3 max-w-3xl text-sm leading-7 text-foreground/65 sm:text-base">
            ...
            {visibleProducts.length > 0 ? (
                    ...
                    <div className="overflow-x-auto pb-2">
                        <div className="flex min-w-max gap-4">
                            {visibleProducts.map(product => (
                                <Card key={product.sku} className="w-72 shrink-0 overflow-hidden border-border shadow-sm sm:w-80">
                                    <CardContent className="flex h-full flex-col p-4">
                                        <div className="relative aspect-square overflow-hidden rounded-2xl bg-muted">
                                            <Image src={product.image} alt={product.title} fill className="object-contain p-3" />
                                        </div>
                                        <div className="mt-4 space-y-2">
                                            <h2 className="line-clamp-2 text-base font-bold text-secondary">{product.title}</h2>
                                            <div className="text-sm text-foreground/60">Brand: {product.brand}</div>
                                            ...
                                            <div className="flex flex-wrap gap-2 text-xs font-semibold text-foreground/60">
                                                <span className="rounded-full bg-muted px-3 py-1">{product.stock}</span>
                                                <span className="rounded-full bg-muted px-3 py-1">Rating {product.rating}</span>
                                            </div>
                                        </div>
                                        <div className="mt-4 grid gap-2 rounded-2xl bg-muted/50 p-3 text-xs text-foreground/70">
                                            ...
                                            <div className="flex items-center justify-between gap-3">
                                                ...
                                                <span className="text-right">{product.weightKg ?? 1} kg</span>
                                            </div>
                                            <div className="flex items-center justify-between gap-3">
                                                <span className="font-semibold text-secondary">COD</span>
                                                <span className="text-right">{product.isNoCOD ? 'Unavailable' : 'Available'}</span>
                                            </div>
                                        </div>
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                {product.id ? (
                                                    <CompareRemoveButton
                                                        ...
                                                        onRemoved={sku => {
                                                            setBackendProducts(current => current.filter(item => item.sku !== sku));
                                                        }}
                                                    />
                                                ) : null}
                                            </div>
                        ...
                        <CardContent className="p-0">
                            <div className="overflow-x-auto">
                                <div className="min-w-max">
                                    ...
                                    >
                                        <div className="p-4 text-sm font-semibold text-secondary">Attribute</div>
                                        {visibleProducts.map(product => (
                                            <div key={product.sku} className="p-4 text-sm font-semibold text-secondary">
                                                {product.brand}
                                            </div>
                                        ))}
                                    </div>
                                    ...
                                    {rows.map(row => (
                                        ...
                                        >
                                            <div className="p-4 text-sm font-semibold text-secondary">{row.label}</div>
                                            {row.values.map((value, index) => (
                                                <div key={`${row.label}-${visibleProducts[index]?.sku ?? index}`} className="p-4 text-sm text-foreground/70">
                                                    {value}
                                                </div>
                                            ))}
                                        </div>
                ...
                <Card className="mt-6 p-8 text-center shadow-sm">
                    <div className="text-lg font-black text-primary">No products added to compare</div>
                    <p className="mt-2 text-sm text-foreground/55">Add products from a product page to build your compare list.</p>
