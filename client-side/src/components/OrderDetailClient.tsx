      ...
      <main className="flex-1 bg-background pb-16">
        <div className="mx-auto w-full max-w-310 px-4 py-6 lg:px-0">
          <Card className="p-6 shadow-sm">
            Order not found.{' '}
            <Link className="font-semibold text-primary" href="/my-account/orders">Back to orders</Link>
          </Card>
        </div>
      </main>
    );
        ...
        <Card className="p-6 shadow-sm sm:p-8">
          ...
          <div className="mt-4 flex flex-wrap items-start justify-between gap-4">
            ...
            <div className="flex flex-wrap gap-2 print:hidden">
              <Button type="button" onClick={() => window.print()} className="h-10 rounded-full bg-secondary px-4 text-sm font-semibold text-secondary-foreground hover:bg-secondary/90">Save PDF</Button>
              <Button type="button" onClick={exportJson} variant="outline" className="h-10 rounded-full border-border px-4 text-sm font-semibold text-foreground/75">Export JSON</Button>
              <Button type="button" onClick={exportCsv} variant="outline" className="h-10 rounded-full border-border px-4 text-sm font-semibold text-foreground/75">Export CSV</Button>
            </div>
          </div>
        </Card>
        ...
        <section className="mt-6 grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
            ...
            <Card className="border border-border bg-muted p-5 print:border-black/15 print:bg-white">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  ...
                  <div className="mt-1 text-sm text-foreground/55">Malamal.com.bd order receipt</div>
                </div>
                ...
                {timeline.map((step, index) => {
                  ...
                  return (
                    <div key={step.key} className={`rounded-2xl border px-4 py-3 text-sm ${isActive ? 'border-primary/30 bg-background' : 'border-border bg-background/70'}`}>
                      <div className={`text-xs uppercase tracking-[0.22em] ${isActive ? 'text-primary' : 'text-foreground/40'}`}>{step.label}</div>
                      <div className="mt-2 font-semibold text-foreground/75">{isCancelled && step.key !== 'Placed' ? 'Skipped' : isActive ? 'Completed' : 'Pending'}</div>
                    </div>
                  );
                })}
          ...
          <Card className="border-0 bg-secondary p-6 text-secondary-foreground shadow-sm">
            ...
            <div className="mt-4 space-y-3 text-sm text-secondary-foreground/80">
              ...
              <div className="flex justify-between font-bold text-secondary-foreground"><span>Total</span><span>{formatMoney(order.total)}</span></div>
            </div>
