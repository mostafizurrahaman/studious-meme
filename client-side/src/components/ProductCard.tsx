...
export function ProductCard({ product }: Props) {
  return (
    <article className="ui-image-card group border border-black/10 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
      <Link href={`/product/${product.sku}`} className="block">
        <div className="relative aspect-square bg-[#f5f6f8] p-2.5">
          <Image
            src={product.image}
            alt={product.title}
            fill
          ...
          />
          {product.badge ? (
            <span className="absolute left-2.5 top-2.5 rounded-full bg-primary px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.12em] text-primary-foreground shadow-sm">
              {product.badge}
            </span>
          ) : null}
      ...
      <div className="space-y-2 p-4">
        <Link href={`/product/${product.sku}`} className="block">
          <h3 className="line-clamp-2 min-h-[2.9rem] text-[13px] font-semibold leading-5 text-black transition hover:text-primary">
            {product.title}
          </h3>
        </Link>
        ...
        <div className="flex gap-2 pt-1">
          ...
          <Button
            asChild
            variant="outline"
            className="h-12 rounded-full border-black/10 px-3 text-[11px] font-semibold text-black/70"
          >
