...
function countItems(
  result: { data?: unknown; meta?: { total?: number }; summary?: { total?: number } } | null | undefined,
): number {
  if (!result) return 0;
...
function renderFeatureList(items: string[]) {
  return (
    <div className="grid gap-3 md:grid-cols-3">
      {items.map(item => (
        <div key={item} className="rounded-2xl border bg-background p-4 text-sm text-foreground/75 shadow-sm">
          {item}
        </div>
      ))}
    </div>
    ...
    <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      {metrics.map(metric => (
        <Card key={metric.label} className="shadow-sm">
          ...
          <CardContent className="pt-0 text-sm text-muted-foreground">{metric.description}</CardContent>
        </Card>
      ))}
    </section>
  );
    ...
    return (
      <div className="space-y-6">
        <Card className="shadow-sm">
          <CardHeader>
            <p className="text-xs font-semibold uppercase tracking-[0.28em] text-primary">{config.eyebrow}</p>
        ...
        <Card className="shadow-sm">
          <CardHeader>
            ...
            <CardDescription>Everything here is pulled from your live account session.</CardDescription>
          </CardHeader>
        ...
        <Card className="shadow-sm">
          <CardHeader>
            ...
            <CardDescription>Direct backend order data for your account.</CardDescription>
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {orders.slice(0, 5).map(order => (
                  <TableRow key={order.orderId}>
                    ...
                    <TableCell>{formatMoney(safeNumber(order.total))}</TableCell>
                  </TableRow>
                ))}
                {orders.length === 0 ? (
                  <TableRow>
    ...
    <div className="space-y-6">
      ...
      <Card className="shadow-sm">
        <CardHeader>
          ...
          <CardDescription>Role-specific responsibilities backed by live backend data.</CardDescription>
        </CardHeader>
        <CardContent>{renderFeatureList(config.responsibilities)}</CardContent>
      </Card>
    </div>
