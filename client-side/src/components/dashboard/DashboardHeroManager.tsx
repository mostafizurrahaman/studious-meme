...
const heroSectionSchema = z.object({
    slides: z.array(heroCardSchema).min(1, { message: 'At least one slide is required!' }),
    features: z.array(heroCardSchema).min(1, { message: 'At least one feature is required!' }),
    isActive: z.boolean(),
});

type HeroCardErrors = Partial<Record<'image' | 'imageAlt' | 'title' | 'description' | 'clickUrl', string>>;
...
function emptyCard() {
    return { image: null as string | File | null, imageAlt: '', title: '', description: '', clickUrl: '' };
}
...
function toErrors(cards: HeroCardForm[]): HeroCardErrors[] {
    return cards.map(card => {
        ...
        return parsed.error.issues.reduce<HeroCardErrors>((acc, issue) => {
            ...
            if (key === 'image' || key === 'imageAlt' || key === 'title' || key === 'description' || key === 'clickUrl') {
...
function updateCardField(
    ...
    setCards(cards.map((item, currentIndex) => (currentIndex === index ? nextCard : item)));
...
export function DashboardHeroManager({ heroes }: { heroes: BackendHeroSection[] }) {
    ...
    const [formErrors, setFormErrors] = useState<HeroSectionErrors>(emptyErrors());
                    ...
                    <div className="grid items-start gap-5 xl:grid-cols-2">
                            ...
                            <div className="grid gap-3 md:grid-cols-2">
                                <div className="space-y-1">
                                    <DashboardInput
                                        ...
                                        onBlur={e =>
                                            setCardErrorAt(errors, index, { ...card, imageAlt: e.target.value }, setErrors)
                                        }
                                    />
                                ...
                                <div className="space-y-1">
                                    <DashboardInput
                                        ...
                                        onBlur={e =>
                                            setCardErrorAt(errors, index, { ...card, title: e.target.value }, setErrors)
                                        }
                                    />
                                ...
                                <div className="space-y-1">
                                    <DashboardInput
                                        ...
                                        onBlur={e =>
                                            setCardErrorAt(errors, index, { ...card, clickUrl: e.target.value }, setErrors)
                                        }
                                    />
                        ...
                        <div className="space-y-2 justify-self-stretch xl:sticky xl:top-4">
                            ...
                            <FileUpload
                                ...
                                onBlur={() => setCardErrorAt(errors, index, { ...card }, setErrors)}
                ...
                <Card className="shadow-sm">
                    ...
                    <CardContent className="space-y-4">
                        {renderCards(
                            ...
                            slidesErrors => setFormErrors({ ...formErrors, slides: slidesErrors }),
                        ...
                        {renderCards(
                            ...
                            featuresErrors => setFormErrors({ ...formErrors, features: featuresErrors }),
                        ...
                        <Card key={hero._id ?? hero.slides[0]?.title ?? 'hero-section'} className="shadow-sm">
                            ...
                            <CardContent className="space-y-4">
                                {renderCards(
                                    ...
                                    slidesErrors => isEditing && setEditingErrors({ ...editingErrors, slides: slidesErrors }),
                                    () => {
                                        if (!hero._id) return;
                                ...
                                {renderCards(
                                    ...
                                    featuresErrors => isEditing && setEditingErrors({ ...editingErrors, features: featuresErrors }),
                                    () => {
                                        if (!hero._id) return;
