...
import { MessageCircleMore, MessageSquare, PhoneCall, X, type LucideIcon } from 'lucide-react';
import {
  useEffect,
  useRef,
  useState,
...
function FloatingLabel({ children, side }: { children: ReactNode; side: 'left' | 'right' }) {
  return (
    <span
...
export function FloatingContactWidget() {
  ...
  const actions: ContactAction[] = [
    {
      ...
      className: 'bg-[#25D366] text-white shadow-[0_10px_20px_rgba(37,211,102,0.28)]',
    },
    {
      label: 'Messenger',
      href: messengerHref,
      icon: MessageSquare,
      className: 'bg-[#0084FF] text-white shadow-[0_10px_20px_rgba(0,132,255,0.22)]',
    },
    {
      label: 'Call',
      href: hotlineHref,
      icon: PhoneCall,
      className: 'bg-sky-500 text-white shadow-[0_10px_20px_rgba(14,165,233,0.22)]',
    },
  ];
  ...
  function clampOffset(nextOffset: { x: number; y: number }, width: number, height: number) {
  ...
  const actualLeft = viewport.width ? viewport.width - RIGHT_GAP - BUTTON_SIZE + clampedOffset.x : 0;
  const actualTop = viewport.height ? viewport.height - BOTTOM_GAP - BUTTON_SIZE + clampedOffset.y : 0;
  const labelSide: 'left' | 'right' =
    viewport.width && actualLeft + BUTTON_SIZE / 2 < viewport.width / 2 ? 'right' : 'left';
  const stackDirection: 'up' | 'down' =
    viewport.height && actualTop + BUTTON_SIZE / 2 < viewport.height / 2 ? 'down' : 'up';
  ...
  const handlePointerMove = (event: ReactPointerEvent<HTMLButtonElement>) => {
    ...
    if (!dragRef.current.dragging && Math.hypot(deltaX, deltaY) > DRAG_THRESHOLD) {
  ...
  return (
    <div ref={containerRef} className="fixed right-3 bottom-28 z-50 sm:right-6 sm:bottom-6 lg:right-8">
      <div
      ...
      >
        ...
        >
          {actions.map(action => {
            ...
            return (
              <div key={action.label} className="group relative flex items-center">
