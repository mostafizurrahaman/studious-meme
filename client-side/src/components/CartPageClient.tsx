...
export function CartPageClient() {
  ...
  const subtotal = useCartStore(state => state.items.reduce((sum, item) => sum + item.unitPrice * item.quantity, 0));
      ...
      <div className="mx-auto w-full max-w-310 px-4 py-6 lg:px-0">
        ...
        <section className="mt-6 grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          <Card className="space-y-4 p-6 shadow-sm">
            {items.length > 0 ? (
              items.map(item => (
                <div key={item.sku} className="flex gap-4 rounded-2xl border border-border p-4">
                  <Link href={item.href} className="relative h-20 w-20 shrink-0 overflow-hidden rounded-2xl bg-muted">
                    <Image src={item.image} alt={item.title} fill className="object-contain p-2" />
                  </Link>
                  <div className="min-w-0 flex-1">
                    <Link href={item.href} className="line-clamp-2 font-semibold text-foreground hover:text-primary">
                      {item.title}
                    </Link>
                    <div className="mt-1 text-sm text-foreground/55">SKU {item.sku} · {item.brand}</div>
                    <div className="mt-2 text-sm font-bold text-primary">{item.unitPriceLabel}</div>
                  </div>
                  <div className="flex flex-col items-end gap-3">
                    <div className="flex items-center gap-2 rounded-full border border-border px-2 py-1">
                      <Button type="button" variant="ghost" onClick={() => decrease(item.sku)} className="h-auto px-2 text-lg leading-none text-foreground/70 hover:bg-transparent">−</Button>
                      <span className="min-w-6 text-center text-sm font-semibold">{item.quantity}</span>
                      <Button type="button" variant="ghost" onClick={() => increase(item.sku)} className="h-auto px-2 text-lg leading-none text-foreground/70 hover:bg-transparent">+</Button>
                    </div>
                    <div className="text-sm font-semibold text-foreground/70">{formatMoney(item.unitPrice * item.quantity)}</div>
                    <Button type="button" variant="link" onClick={() => remove(item.sku)} className="h-auto p-0 text-xs font-semibold text-primary">
                      Remove
                    </Button>
                  </div>
                </div>
              ...
              <div className="rounded-3xl border border-dashed border-border p-8 text-center">
                ...
                <p className="mt-2 text-sm text-foreground/55">Start browsing the catalog and add products to save them here.</p>
            ...
            <Card className="bg-muted p-4">
              <div className="flex flex-wrap items-center gap-3">
                ...
                {appliedCoupon ? (
                  <Button type="button" onClick={() => { clearCoupon(); setCouponMessage(''); }} variant="outline" className="h-11 rounded-full border-border px-5 text-sm font-semibold text-foreground/70">
                    Remove
                  </Button>
                ) : null}
              </div>
          ...
          <Card className="border-0 bg-secondary p-6 text-secondary-foreground shadow-sm">
            ...
            <div className="mt-6 grid gap-3">
              ...
              <Button asChild variant="outline" className="h-11 rounded-full border-white/20 bg-white px-6 text-sm font-bold text-secondary hover:bg-white/90">
                <Link href="/checkout">Proceed to checkout</Link>
              </Button>
              <Button type="button" onClick={clear} variant="outline" className="h-11 rounded-full border-white/20 px-6 text-sm font-bold text-white hover:bg-white/10">
                Clear cart
              </Button>
            </div>
          </Card>
        ...
        <Card className="mt-6 p-6 shadow-sm sm:p-8">
          <div className="flex flex-wrap items-end justify-between gap-4">
            ...
            <div className="rounded-full bg-muted px-4 py-2 text-sm font-semibold text-foreground/70">Persisted checkout form</div>
          </div>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            ...
            <label className="grid gap-2 text-sm font-semibold text-foreground lg:col-span-2">
              Delivery address
              <Textarea value={checkout.address} onChange={event => updateCheckout('address', event.target.value)} className="min-h-28" />
            </label>
            <label className="grid gap-2 text-sm font-semibold text-foreground lg:col-span-2">
              Order note
              <Textarea value={checkout.note} onChange={event => updateCheckout('note', event.target.value)} className="min-h-24" />
            </label>
            <label className="grid gap-2 text-sm font-semibold text-foreground lg:col-span-2">
              Payment method
              <select value={checkout.payment} onChange={event => updateCheckout('payment', event.target.value)} className="h-11 rounded-2xl border border-input bg-background px-4 outline-none">
                {['Cash on delivery', 'Bank transfer', 'bKash', 'Nagad'].map(option => <option key={option}>{option}</option>)}
              </select>
            </label>
          </div>
        </Card>
