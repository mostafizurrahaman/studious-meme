...
const normalizeCouponPayload = (payload: Partial<CouponMutationPayload>) => {
    ...
    if (typeof payload.minSubtotal === 'number' && Number.isFinite(payload.minSubtotal)) {
        normalized.minSubtotal = payload.minSubtotal;
    }

    if (payload.expiresAt instanceof Date || typeof payload.expiresAt === 'string') {
...
export const getAllCoupons = async (
    ...
    return requestBackendJson<BackendEnvelope<Coupon[]>>(`/coupon/admin${query ? `?${query}` : ''}`, {
        method: 'GET',
        token: accessToken ?? undefined,
    });
};

export const getCouponById = async (couponId: string): Promise<BackendEnvelope<Coupon>> => {
    const accessToken = await getValidAccessTokenForServerHandlerGet();

    return requestBackendJson<BackendEnvelope<Coupon>>(`/coupon/admin/${couponId}`, {
        method: 'GET',
        token: accessToken ?? undefined,
    });
};

export const createCoupon = async (payload: CouponMutationPayload): Promise<BackendEnvelope<Coupon>> => {
    const accessToken = await getValidAccessTokenForServerActions();
    const result = await requestBackendJson<BackendEnvelope<Coupon>>('/coupon/admin', {
        method: 'POST',
...
export const updateCoupon = async (
    ...
    const accessToken = await getValidAccessTokenForServerActions();
    const result = await requestBackendJson<BackendEnvelope<Coupon>>(`/coupon/admin/${couponId}`, {
        method: 'PATCH',
        body: normalizeCouponPayload(payload),
        token: accessToken ?? undefined,
    });
...
export const updateCouponStatus = async (
    ...
    const accessToken = await getValidAccessTokenForServerActions();
    const result = await requestBackendJson<BackendEnvelope<Coupon>>(`/coupon/admin/${couponId}/status`, {
        method: 'PATCH',
        body: { isActive },
        token: accessToken ?? undefined,
    });
...
export const deleteCoupon = async (couponId: string): Promise<BackendEnvelope<Coupon>> => {
    const accessToken = await getValidAccessTokenForServerActions();
    const result = await requestBackendJson<BackendEnvelope<Coupon>>(`/coupon/admin/${couponId}`, {
        method: 'DELETE',
        token: accessToken ?? undefined,
