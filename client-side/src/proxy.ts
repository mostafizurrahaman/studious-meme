...
const BACKEND_FULL_URL = process.env.NEXT_PUBLIC_BACKEND_FULL_URL?.replace(/\/$/, '') ?? '';
...
export async function proxy(request: NextRequest) {
    ...
    const currentRefreshToken = request.cookies.get('refreshToken')?.value ?? null;
    ...
    const buildResponse = (kind: 'next' | 'redirect', targetPath?: string, clearCookies = false) => {
        const response =
            kind === 'redirect' && targetPath ? NextResponse.redirect(new URL(targetPath, request.url)) : NextResponse.next();
    ...
    if (pathname === '/my-account' || pathname.startsWith('/my-account/')) {
        return buildResponse('redirect', getDashboardPathByRole(session.role) ?? '/dashboard');
    }
