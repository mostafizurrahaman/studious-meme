...
export function comparisonHistoryRecordToProduct(record: ComparisonHistoryRecord): Product | null {
...
export function getComparisonHistoryProductId(record: ComparisonHistoryRecord): string | null {
...
export function canAddToCompare(items: Product[], product: Product): CompareCheck {
    ...
    if (items.length > 0 && getCategoryKey(items[0]) !== getCategoryKey(product)) {
        return {
            allowed: false,
...
export function buildCompareSpecRows(products: Product[]): CompareSpecRow[] {
    return [
        ...
        { label: 'Old price', values: products.map(product => product.oldPrice ?? '—') },
        ...
        { label: 'Weight', values: products.map(product => `${product.weightKg ?? 1} kg`) },
        {
            label: 'COD',
            values: products.map(product => (product.isNoCOD ? 'Unavailable' : 'Available')),
        },
    ];
}
...
export function compareProductsSignature(products: Product[]): string {
    return products
        .map(product => [product.sku, product.price, product.oldPrice ?? '', product.stock, product.rating].join('|'))
        .join('::');
}
