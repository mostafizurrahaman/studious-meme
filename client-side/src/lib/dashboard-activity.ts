...
const getToneKey = (record: DashboardActivityRecord) => getToneClass(getUserKey(record.user));
...
export function buildDashboardActivityCards(records: DashboardActivityRecord[]): DashboardActivityCard[] {
    ...
    [...records]
        ...
        if (action === 'clear' || !record.productSnapshot) {
            ...
            for (const state of groups.values()) {
                if (state.card.userEmail === user.email && state.card.isActive) {
                    ...
                    state.card.removedAt = state.card.removedAt ?? new Date(eventTime).toISOString();
                }
            }
            ...
            groups.set(clearKey, {
                card: {
                    ...
                    userPhone: typeof record.user === 'object' ? record.user.phone : undefined,
                    userImage: typeof record.user === 'object' ? record.user.image : undefined,
                    title: 'Cart cleared',
                    brand: 'Cart',
                    sku: 'CLEAR',
                    image: '/icon.png',
                    addedAt: action === 'clear' ? new Date(eventTime).toISOString() : undefined,
        ...
        if (!existing) {
            groups.set(key, {
                card: {
                    ...
                    addedAt: action === 'add' ? new Date(eventTime).toISOString() : undefined,
                    updatedAt: new Date(eventTime).toISOString(),
                    removedAt: action === 'remove' ? new Date(eventTime).toISOString() : undefined,
                    lastAction: action,
                    eventCount: 1,
        ...
        nextCard.updatedAt = new Date(Math.max(existing.sortAt, eventTime)).toISOString();
        nextCard.eventCount += 1;
        nextCard.lastAction = action;
        nextCard.isActive = action !== 'remove';
        nextCard.quantity = typeof record.quantity === 'number' ? record.quantity : nextCard.quantity;
