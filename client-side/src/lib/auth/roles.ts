...
export function normalizeRoleSegment(segment: string | null | undefined): DashboardRouteSegment | null {
    if (!segment) return null;

    const normalized = segment.trim().toLowerCase().replace(/_/g, '-') as DashboardRouteSegment;
    return normalized in SEGMENT_TO_ROLE ? normalized : null;
}

export function getDashboardSegmentByRole(role: string | null | undefined): DashboardRouteSegment | null {
    const normalized = normalizeRole(role);
    return normalized ? ROLE_TO_SEGMENT[normalized] : null;
}

export function getDashboardPathByRole(role: string | null | undefined): string | null {
    const segment = getDashboardSegmentByRole(role);
    return segment ? `/dashboard/${segment}` : null;
}

export function getProfilePathByRole(role: string | null | undefined): string | null {
    const dashboardPath = getDashboardPathByRole(role);
    return dashboardPath ? `${dashboardPath}/profile` : null;
}

export function getAdminsPathByRole(role: string | null | undefined): string | null {
    return normalizeRole(role) === 'SUPER_ADMIN' ? '/dashboard/super-admin/admins' : null;
}
...
export function isDashboardPathForRole(role: string | null | undefined, pathname: string): boolean {
    const dashboardPath = getDashboardPathByRole(role);
...
export function isAllowedDashboardPath(role: string | null | undefined, pathname: string): boolean {
    return isDashboardRootPath(pathname) || isDashboardPathForRole(role, pathname);
}

export function getCanonicalRolePathFromSegment(segment: string | null | undefined): string | null {
...
export function getCanonicalRoleFromSegment(segment: string | null | undefined): AuthRole | null {
    const normalizedSegment = normalizeRoleSegment(segment);
