...
describe('auth role helpers', () => {
    ...
    it('maps roles to profile and admin paths', () => {
        expect(getProfilePathByRole('SUPER_ADMIN')).toBe('/dashboard/super-admin/profile');
        expect(getAdminsPathByRole('SUPER_ADMIN')).toBe('/dashboard/super-admin/admins');
    ...
    it('checks dashboard path membership and canonical mappings', () => {
        ...
        expect(isDashboardRootPath('/dashboard/')).toBe(true);
        expect(isDashboardPathForRole('ADMIN', '/dashboard/admin/products')).toBe(true);
        expect(isDashboardPathForRole('ADMIN', '/dashboard/user')).toBe(false);
        expect(isAllowedDashboardPath('USER', '/dashboard/user/profile')).toBe(true);
        expect(getCanonicalRolePathFromSegment('super_admin')).toBe('/dashboard/super-admin');
