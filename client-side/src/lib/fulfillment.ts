...
export function deriveShippingZone(city?: string, address?: string): ShippingZone {
...
export function calculateShippingCharge({ totalWeightKg, zone }: { totalWeightKg: number; zone: ShippingZone }) {
...
export function calculateCodEligibility(items: Array<Pick<FulfillmentItem, 'isNoCOD'>>, subtotal: number) {
...
export function calculateFulfillmentSummary({
    ...
    const totalWeightKg = Number(
        items.reduce((sum, item) => sum + (Number.isFinite(item.weightKg ?? 1) ? (item.weightKg ?? 1) : 1) * item.quantity, 0).toFixed(2),
    );
