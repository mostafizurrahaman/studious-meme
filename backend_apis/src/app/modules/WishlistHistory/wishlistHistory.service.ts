...
const addWishlistItemIntoDB = async (user: IUser, productId: string) => {
    ...
    await WishlistHistoryEventModel.create({ user: user._id, product: product._id, productSnapshot: snapshot, action: 'add' });
...
const removeWishlistItemFromDB = async (user: IUser, productId: string) => {
    ...
    const existing = await WishlistHistoryModel.findOne({ user: user._id, product: productObjectId }).lean();
    await WishlistHistoryModel.findOneAndDelete({ user: user._id, product: productObjectId });
...
const getWishlistInsightsFromDB = async () => {
    const [categorySummary, productSummary, userSummary, total] = await Promise.all([
        WishlistHistoryEventModel.aggregate([
            { $group: { _id: '$productSnapshot.category', count: { $sum: 1 }, users: { $addToSet: '$user' } } },
            { $project: { _id: 0, category: '$_id', count: 1, userCount: { $size: '$users' } } },
            { $sort: { count: -1 } },
        ]),
        WishlistHistoryEventModel.aggregate([
            { $group: { _id: '$productSnapshot.title', count: { $sum: 1 }, category: { $first: '$productSnapshot.category' } } },
