...
const couponItemSchema = z.object({
    unitPrice: z.number({ error: 'Unit price is required!' }).nonnegative({ message: 'Unit price must be at least 0!' }),
    quantity: z.number({ error: 'Quantity is required!' }).int().positive({ message: 'Quantity must be at least 1!' }),
...
const couponBaseObject = z.object({
    ...
    discountValue: z.number({ error: 'Discount value is required!' }).nonnegative({ message: 'Discount value must be at least 0!' }),
    minSubtotal: z.number().nonnegative({ message: 'Minimum subtotal must be at least 0!' }).optional(),
...
const couponUpdateBodySchema = couponBaseObject.partial().superRefine((value, ctx) => {
...
const couponVerifySchema = z.object({
    body: z.object({
        ...
        items: z.array(couponItemSchema).min(1, { message: 'At least one item is required!' }),
...
const couponStatusSchema = z.object({
    params: z.object({
        couponId: z.string({ error: 'Coupon ID is required!' }).trim().min(1, { message: 'Coupon ID is required!' }),
    }),
    body: z.object({
