...
import { buildCouponCheckoutBase, buildCouponInvalidSummary, normalizeCouponCode, resolveCouponSummary, serializeCoupon } from './coupon.utils';
import type { TCouponCheckoutInput, TCouponCheckoutSummary, TCouponCreatePayload } from './coupon.interface';
...
const buildCouponFilter = (query: CouponQuery) => {
    ...
    const discountType = typeof query.discountType === 'string' ? query.discountType.trim() : '';
...
const createCouponIntoDB = async (payload: TCouponCreatePayload) => {
    ...
    const coupon = await CouponModel.create({
        ...normalized,
        discountValue: normalized.discountType === 'FREE_SHIPPING' ? 0 : normalized.discountValue,
...
const updateCouponIntoDB = async (couponId: string, payload: Partial<TCouponCreatePayload>) => {
    ...
    const nextDiscountType = normalized.discountType ?? existingCoupon.discountType;
    const nextDiscountValue =
        typeof normalized.discountValue === 'number' ? normalized.discountValue : existingCoupon.discountValue;
    ...
    const coupon = await CouponModel.findByIdAndUpdate(
        couponId,
        {
            ...
            ...(normalized.discountType ? { discountType: normalized.discountType } : {}),
            ...(normalized.discountType || typeof normalized.discountValue === 'number'
                ? {
                      discountValue: nextDiscountType === 'FREE_SHIPPING' ? 0 : nextDiscountValue,
                  }
                : {}),
            ...(typeof normalized.minSubtotal === 'number' ? { minSubtotal: normalized.minSubtotal } : {}),
            ...(normalized.expiresAt ? { expiresAt: normalized.expiresAt } : {}),
            ...(typeof normalized.isActive === 'boolean' ? { isActive: normalized.isActive } : {}),
        },
...
const invalidCouponSummary = (base: ReturnType<typeof buildCouponCheckoutBase>, message: string): TCouponCheckoutSummary =>
    buildCouponInvalidSummary(base, message);

const calculateCouponCheckoutSummary = async (payload: TCouponCheckoutInput): Promise<TCouponCheckoutSummary> => {
    ...
    const coupon = await CouponModel.findOne({ code: normalizeCouponCode(couponCode) }).lean();
...
const verifyCouponFromDB = async (payload: TCouponCheckoutInput) => calculateCouponCheckoutSummary(payload);
