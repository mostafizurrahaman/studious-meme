import { calculateCodEligibility, calculateShippingCharge, deriveShippingZone, getTotalWeightKg } from '../Order/order.utils';
import type { TCouponCheckoutInput, TCouponCheckoutSummary, TCouponDiscountType, TCouponRecord } from './coupon.interface';
...
export const buildCouponCheckoutBase = (payload: Pick<TCouponCheckoutInput, 'items' | 'city' | 'address'>) => {
    const subtotal = payload.items.reduce((sum, item) => sum + item.unitPrice * item.quantity, 0);
    const totalWeightKg = getTotalWeightKg(
        payload.items.reduce((sum, item) => sum + (Number.isFinite(item.weightKg ?? 1) ? (item.weightKg ?? 1) : 1) * item.quantity, 0),
    );
    const shippingZone = deriveShippingZone(payload.city, payload.address);
    const baseShippingCharge = calculateShippingCharge({ totalWeightKg, zone: shippingZone });
...
export const buildCouponInvalidSummary = (base: ReturnType<typeof buildCouponCheckoutBase>, message: string): TCouponCheckoutSummary => {
