...
const buildAuthHeader = (): PortPosAuth => {
    ...
    const token = crypto
        ...
        .digest('hex');
    const authorization = Buffer.from(`${String(config.portpos.app_key)}:${token}`).toString('base64');
...
const buildFrontendUrl = (path: string) => `${String(config.urls.frontend_app).replace(/\/$/, '')}${path}`;

const buildBackendUrl = (path: string) => `${String(config.urls.backend_public).replace(/\/$/, '')}${path}`;

const extractGatewayMessage = (
    payload: PortPosInvoiceResponse | PortPosInvoiceDetailsResponse | PortPosRefundResponse,
) => {
    if (payload.message) {
...
const requestPortPos = async <T>(path: string, init: PortPosRequestInit): Promise<T> => {
    ensureConfig();
    ...
    const requestFailed =
        !response.ok ||
        (payload && typeof payload === 'object' && 'result' in payload && payload.result === false);

    if (requestFailed) {
        throw new AppError(
            httpStatus.BAD_GATEWAY,
            extractGatewayMessage(
                payload as PortPosInvoiceResponse | PortPosInvoiceDetailsResponse | PortPosRefundResponse,
            ),
        );
    }
...
const getRedirectUrl = (orderId: string, kind: 'success' | 'fail' | 'cancel') => {
...
const createInvoice = async (order: PortPosOrder, user: IUser) => {
    ...
    const payload = {
        order: {
            ...
            ipn_url: config.portpos.ipn_url ?? buildBackendUrl('/api/v1/payment/portpos/ipn'),
            reference: order.orderId,
            validity: 30,
        },
        product: {
            name: order.items[0]?.title ?? `Order ${order.orderId}`,
            description: order.items.map(item => `${item.title} x ${item.quantity}`).join(', '),
        },
        billing: {
            customer: {
    ...
    const response = await requestPortPos<PortPosInvoiceResponse>(config.portpos.payment_url ?? '/invoice', {
        method: 'POST',
        body: JSON.stringify(payload),
    });

    const invoiceId = response.data?.invoice_id;
    const paymentUrl = response.data?.action?.url ?? response.data?.order?.redirect_url;

    if (!invoiceId || !paymentUrl) {
        throw new AppError(httpStatus.BAD_GATEWAY, 'PortPOS did not return a payment URL.');
    }
...
const verifyInvoice = async (invoiceId: string) => {
    ...
    return requestPortPos<PortPosInvoiceDetailsResponse>(`/invoice/${encodeURIComponent(invoiceId)}`, {
        method: 'GET',
    });
};
...
const refundPayment = async (invoiceId: string, amount: number) => {
    if (!invoiceId) {
        throw new AppError(httpStatus.BAD_REQUEST, 'Invoice ID is required for refunds.');
    }

    return requestPortPos<PortPosRefundResponse>(`/invoice/refund/${encodeURIComponent(invoiceId)}`, {
        method: 'POST',
        body: JSON.stringify({
            refund: {
                amount,
