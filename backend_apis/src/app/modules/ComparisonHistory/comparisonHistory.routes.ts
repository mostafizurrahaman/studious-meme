...
router
    .route('/admin/summary')
    .get(auth(ROLE.ADMIN, ROLE.SUPER_ADMIN), adminLimiter, ComparisonHistoryController.getComparisonInsights);

router
    .route('/:productId')
    .delete(auth(ROLE.USER, ROLE.ADMIN, ROLE.SUPER_ADMIN), actionLimiter, ComparisonHistoryController.removeComparisonItem);
