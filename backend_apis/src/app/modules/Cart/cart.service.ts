...
const getOrCreateCart = async (userId: Types.ObjectId) => {
    ...
    return CartModel.create({ user: userId, items: [], subtotal: 0, totalItems: 0 });
};

const recalculateCart = (cart: { items: Array<{ quantity: number; priceSnapshot: number }> }) => {
    const subtotal = cart.items.reduce((sum, item) => sum + item.quantity * item.priceSnapshot, 0);
    const totalItems = cart.items.reduce((sum, item) => sum + item.quantity, 0);
    return { subtotal, totalItems };
};

const addCartItemIntoDB = async (user: IUser, productId: string, quantity = 1) => {
    ...
    if (existing) {
        ...
        if (cart.items.length >= MAX_CART_ITEMS) {
            throw new AppError(httpStatus.BAD_REQUEST, `You can add up to ${MAX_CART_ITEMS} distinct products to cart.`);
        }

        cart.items.push({ product: product._id as never, quantity, priceSnapshot: snapshot.price, productSnapshot: snapshot });
    }
    ...
    await cart.save();
    await CartHistoryModel.create({ user: user._id, product: product._id, action: 'add', quantity, productSnapshot: snapshot });
    return cart.toObject();
};

const updateCartItemIntoDB = async (user: IUser, productId: string, quantity: number) => {
    const cart = await getOrCreateCart(user._id);
    const item = cart.items.find(entry => String(entry.product) === String(toObjectId(productId)));
    ...
    if (item.quantity <= 0) {
        cart.items = cart.items.filter(entry => String(entry.product) !== String(toObjectId(productId)));
    }
...
const removeCartItemFromDB = async (user: IUser, productId: string) => {
    ...
    const removed = cart.items.find(entry => String(entry.product) === String(productObjectId));
    cart.items = cart.items.filter(entry => String(entry.product) !== String(productObjectId));
    const { subtotal, totalItems } = recalculateCart(cart);
    cart.subtotal = subtotal;
    cart.totalItems = totalItems;
    await cart.save();
...
const getCartInsightsFromDB = async () => {
    const [categorySummary, productSummary, userSummary, total] = await Promise.all([
        CartHistoryModel.aggregate([
            ...
            { $group: { _id: '$productSnapshot.category', count: { $sum: { $ifNull: ['$quantity', 1] } }, users: { $addToSet: '$user' } } },
            { $project: { _id: 0, category: '$_id', count: 1, userCount: { $size: '$users' } } },
            { $sort: { count: -1 } },
        ]),
        CartHistoryModel.aggregate([
            { $match: { action: { $in: ['add', 'update', 'remove'] } } },
            { $group: { _id: '$productSnapshot.title', count: { $sum: { $ifNull: ['$quantity', 1] } }, category: { $first: '$productSnapshot.category' } } },
        ...
        CartHistoryModel.aggregate([
            ...
            { $group: { _id: '$user', count: { $sum: { $ifNull: ['$quantity', 1] } } } },
            { $project: { _id: 0, user: '$_id', count: 1 } },
            { $sort: { count: -1 } },
            { $limit: 10 },
        ]),
        CartHistoryModel.aggregate([{ $group: { _id: null, total: { $sum: { $ifNull: ['$quantity', 1] } } } }]),
    ]);
