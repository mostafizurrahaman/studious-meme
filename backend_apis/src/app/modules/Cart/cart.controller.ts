...
const addCartItem = asyncHandler(async (req, res) => {
    const result = await CartService.addCartItemIntoDB(req.user, req.body.productId, req.body.quantity ?? 1);
...
const updateCartItem = asyncHandler(async (req, res) => {
    const result = await CartService.updateCartItemIntoDB(req.user, req.body.productId, req.body.quantity);
...
const removeCartItem = asyncHandler(async (req, res) => {
    const result = await CartService.removeCartItemFromDB(req.user, getSingleParam(req.params.productId));
