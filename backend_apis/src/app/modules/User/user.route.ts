...
import { actionLimiter, adminLimiter, auth, authLimiter, burstProtection, duplicateSubmissionGuard, validateRequest } from '../../middlewares';
...
router.route('/access-token').get(authLimiter, UserController.getNewAccessToken);
...
router.route('/delete-account').delete(auth(ROLE.USER), actionLimiter, UserController.deleteSpecificUserAccount);

// 16. adminGetAllUsers
router.route('/admin-get-all').get(auth(ROLE.ADMIN, ROLE.SUPER_ADMIN), adminLimiter, UserController.adminGetAllUsers);
...
// 18. adminDeleteUser
router.route('/admin-users/:userId').delete(auth(ROLE.ADMIN, ROLE.SUPER_ADMIN), adminLimiter, UserController.adminDeleteUser);
