...
const createOrderSchema = z.object({
    body: z.object({
        ...
        customer: z.object({
            name: z.string({ error: 'Name is required!' }).trim().min(2, { message: 'Name must be at least 2 characters long!' }),
            phone: z.string({ error: 'Phone is required!' }).trim().min(6, { message: 'Phone must be at least 6 characters long!' }),
            email: z.string().trim().email().optional().or(z.literal('')),
            address: z.string({ error: 'Address is required!' }).trim().min(5, { message: 'Address must be at least 5 characters long!' }),
            city: z.string({ error: 'City is required!' }).trim().min(2, { message: 'City must be at least 2 characters long!' }),
